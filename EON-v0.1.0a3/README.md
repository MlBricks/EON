...
EON is an MLBricks architecture package.
...
EON has no SOUP Observer Memory/Fusion block; its recurrent generation cache therefore contains only the mixer states.

## Training fast path

EON uses the same hot-path policy as optimized SOUP:
...
These are execution-path optimizations only; EON equations, trainable parameters, and checkpoint keys are unchanged.
